...
async function submitTx(request, txName, ...args) {
  try {
    ...
    return utils.setUserContext(request.username, request.password).then(
      (contract) => {
        ...
        return utils.submitTx.apply("unused", args).then(
          (buffer) => {
            return buffer;
          },
          (error) => {
...
apiRouter.route("/orders").get(function (request, response) {
  submitTx(request, "queryAllOrders", "").then(
    (queryOrderResponse) => {
...
apiRouter.route("/orders/:id").get(function (request, response) {
  submitTx(request, "queryOrder", request.params.id).then(
    (queryOrderResponse) => {
      // process response
      let order = Order.fromBuffer(queryOrderResponse);
      console.log(
        `order ${order.orderId} : price = ${order.price}, quantity = ${order.quantity}, producer = ${order.producerId}, consumer = ${order.retailerId}, trackingInfo = ${order.trackingInfo}, state = ${order.currentOrderState}`
      );
      response.status(STATUS_SUCCESS);
      response.send(order);
    },
...
apiRouter.route("/orders").post(function (request, response) {
  submitTx(request, "orderProduct", JSON.stringify(request.body)).then(
    (result) => {
      // process response
    ...
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          STATUS_SERVER_ERROR,
          "There was a problem when placing the order."
        )
      );
    }
  );
...
apiRouter.route("/order-history/:id").get(function (request, response) {
  submitTx(request, "getOrderHistory", request.params.id).then(
    (orderHistoryResponse) => {
      console.log(
        "\n>> Process getOrderHistory response <<",
        orderHistoryResponse
      );
      //  response is already a string;  not a buffer
    ...
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          STATUS_SERVER_ERROR,
          "There was a problem fetching history for order, ",
          request.params.id
        )
      );
    }
...
apiRouter.route("/receive-order/:id").put(function (request, response) {
  submitTx(request, "receiveOrder", request.params.id).then(
    (receiveOrderResponse) => {
      // process response
      console.log(">> Process ReceiveOrder transaction <<");
      let order = Order.fromBuffer(receiveOrderResponse);
      console.log(
        `order ${order.orderId} : state = ${order.currentOrderState}`
      );
      response.status(STATUS_SUCCESS);
      response.send(order);
    },
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          STATUS_SERVER_ERROR,
          "There was a problem in receiving order, ",
          request.params.id
        )
      );
    }
...
apiRouter.route("/assign-shipper/:id").put(function (request, response) {
  submitTx(
    request,
    "assignShipper",
    request.params.id,
    request.query.shipperid
  ).then(
    (assignShipperResponse) => {
      console.log(">> Process AssignShipper transaction <<");
      let order = Order.fromBuffer(assignShipperResponse);
      console.log(
        `order ${order.orderId} : shipper = ${order.shipperId}, state = ${order.currentOrderState}`
      );
      response.status(STATUS_SUCCESS);
      response.send(order);
    },
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          STATUS_SERVER_ERROR,
          "There was a problem in assigning shipper for order, ",
          request.params.id
        )
      );
    }
...
apiRouter
  .route("/create-shipment-for-order/:id")
  .put(function (request, response) {
    submitTx(
      ...
      (createShipmentResponse) => {
        ...
        console.log(
          `order ${order.orderId} : trackingInfo = ${order.trackingInfo}, state = ${order.currentOrderState}`
        );
        response.status(STATUS_SUCCESS);
        response.send(order);
      },
      (error) => {
        ...
        response.send(
          utils.prepareErrorResponse(
            error,
            STATUS_SERVER_ERROR,
            "There was a problem in creating shipment for order," +
              request.params.id
          )
        );
      }
...
apiRouter.route("/transport-shipment/:id").put(function (request, response) {
  submitTx(request, "transportShipment", request.params.id).then(
    (transportShipmentResponse) => {
    ...
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          STATUS_SERVER_ERROR,
          "There was a problem in initiating shipment for order," +
            request.params.id
        )
      );
    }
...
// This changes the status on the order
apiRouter.route("/receive-shipment/:id").put(function (request, response) {
  submitTx(request, "receiveShipment", request.params.id).then(
    ...
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          STATUS_SERVER_ERROR,
          "There was a problem in receiving shipment for order," +
            request.params.id
        )
      );
    }
...
apiRouter.route("/orders/:id").delete(function (request, response) {
  submitTx(request, "deleteOrder", request.params.id).then(
    (deleteOrderResponse) => {
      // process response
...
apiRouter.route("/register-user").post(function (request, response) {
  try {
    ...
    getUsernamePassword(request).then(
      (request) => {
        ...
        utils.registerUser(userId, userPwd, userType, request.username).then(
          (result) => {
            response.status(STATUS_SUCCESS);
...
apiRouter.route("/enroll-user/").post(function (request, response) {
  let userType = request.body.usertype;
  //  retrieve username, password of the called from authorization header
  getUsernamePassword(request).then((request) => {
    utils.enrollUser(request.username, request.password, userType).then(
      (result) => {
        response.status(STATUS_SUCCESS);
        response.send(result);
      },
      (error) => {
        ...
        response.send(
          utils.prepareErrorResponse(
            ...
            "User, " +
              request.username +
              " could not be enrolled. Check that user is registered."
          )
        );
      }
    );
...
*/
apiRouter.route("/is-user-enrolled/:id").get(function (request, response) {
  ...
  getUsernamePassword(request).then(
    (request) => {
      ...
      utils.isUserEnrolled(userId).then(
        (result) => {
          response.status(STATUS_SUCCESS);
          response.send(result);
        },
    ...
    (error) => {
      ...
      response.send(
        utils.prepareErrorResponse(
          error,
          INVALID_HEADER,
          "Invalid header; Error checking enrollment for user, " +
            request.params.id
        )
      );
    }
...
apiRouter.route("/users").get(function (request, response) {
  getUsernamePassword(request).then(
    (request) => {
      utils.getAllUsers(request.username).then(
        (result) => {
          response.status(STATUS_SUCCESS);
          response.send(result);
        },
...
apiRouter.route("/users/:id").get(function (request, response) {
  //  Get admin username and pwd from request header
  //  Only admin can call this api; this is not verified here;
  //  Possible future enhancement
  getUsernamePassword(request).then(
    (request) => {
      utils.isUserEnrolled(request.params.id).then(
        (result1) => {
          if (result1 == true) {
            utils.getUser(request.params.id, request.username).then(
              (result2) => {
                response.status(STATUS_SUCCESS);
