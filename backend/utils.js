...
utils.prepareErrorResponse = (error, code, message) => {
  ...
  console.log("utils:prepareErrorResponse(): " + message);
  console.log(result);
  return result;
};
  ...
  try {
    ...
    if (platform == "IBP") {
      configdata = JSON.parse(
        fs.readFileSync("../gateway/ibp/config.json", "utf8")
      );
      ...
      // PLATFORM = LOCAL
      configdata = JSON.parse(
        fs.readFileSync("../gateway/local/config.json", "utf8")
      );
      console.log("Platform = " + platform);
      bLocalHost = true;
    }
    ...
    console.log(">> Connecting into Fabric gateway <<");
    ...
    console.log(
      "Use " + configdata["smart_contract_name"] + " smart contract."
    );
    ...
    console.log(
      "Error when try to connecting into the Fabric network. " +
        error.toString()
    );
  } finally {
  }
...
utils.events = async () => {
  ...
  if (peers.length == 0) {
    throw new Error(
      "Error after calling channel.getChannelPeers(): Channel has no peers !"
    );
  }
  ...
  let event_monitor = new Promise(
    (resolve, reject) => {
      /*  Sample usage of registerChaincodeEvent
        ...
        */
      var regid = channel_event_hub.registerChaincodeEvent(
        ...
        (err) => {
          ...
          reject(
            new Error(
              "There was an problem with the eventhub in the registerTxEvent ::" +
                err
            )
          );
        },
...
utils.submitTx = async (contract, txName, ...args) => {
  ...
  return result.then(
    (response) => {
      ...
      console.log("Utils: Transaction has been submitted successfully");
      return Promise.resolve(response.toString());
    },
    (error) => {
      console.log("Utils: Error: " + error.toString());
...
utils.registerUser = async (userid, userpwd, usertype, adminIdentity) => {
  ...
  return ca.register(newUserDetails, gateway.getCurrentIdentity()).then(
    (newPwd) => {
    ...
    (error) => {
      console.log(
        "Error in when trying to register;  ERROR returned: " + error.toString()
      );
      return Promise.reject(error);
    }
  );
...
utils.enrollUser = async (userid, userpwd, usertype) => {
  ...
  return ca.enroll(newUserDetails).then(
    (enrollment) => {
      ...
      return wallet.import(userid, identity).then(
        (notused) => {
          return console.log(
            "Successfully enrolling the user, " +
              userid +
              " and imported it into the wallet"
          );
        },
        (error) => {
          console.log("Error when trying to import\n" + error.toString());
          throw error;
        }
      );
    },
    (error) => {
      console.log("Error when trying to enroll " + error.toString());
      throw error;
    }
  );
};
...
utils.setUserContext = async (userid, pwd) => {
  ...
  if (!userExists) {
    console.log(
      "An identity with the user: " + userid + " does not exist in the wallet"
    );
  ...
  try {
    // Connect to gateway using application specified parameters
    console.log("Connecting to the Fabric gateway with userid:" + userid);
...
utils.isUserEnrolled = async (userid) => {
  return wallet.exists(userid).then(
    (result) => {
      return result;
    },
    (error) => {
...
utils.getUser = async (userid, adminIdentity) => {
  ...
  if (userid == "admin") {
    ...
    // look through user attributes for "usertype"
    let j = 0;
...
utils.getAllUsers = async (adminIdentity) => {
  ...
  for (var i = 0; i < identities.length; i++) {
    ...
    if (tmp.id == "admin") tmp.usertype = tmp.id;
    else {
