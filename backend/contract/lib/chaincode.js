...
class Chaincode extends Contract {
  constructor() {
    ...
    super("org.supplychainnet.chaincode");
  }
  ...
  async orderProduct(ctx, args) {
    ...
    if (
      userType != "admin" && // admin only has access as a precaution.
      userType != "producer" &&
      userType != "retailer"
    )
  ...
  async receiveOrder(ctx, orderId) {
    ...
    if (
      userId != "admin" && // admin only has access as a precaution.
      userId != order.producerId
    )
      throw new Error(
        `${userId} does not have access to receive order ${orderId}`
      );
  ...
  async assignShipper(ctx, orderId, newShipperId) {
    ...
    if (
      ...
      throw new Error(
        `${userId} does not have access to assign a shipper to order ${orderId}`
      );
  ...
  async createShipment(ctx, orderId, newTrackingInfo) {
    ...
    if (
      ...
      throw new Error(
        `${userId} does not have access to create a shipment for order ${orderId}`
      );
  ...
  async transportShipment(ctx, orderId) {
    ...
    if (
      ...
      // This transaction should only be invoked by
      throw new Error(
        `${userId} does not have access to transport shipment for order ${orderId}`
      );
  ...
  async receiveShipment(ctx, orderId) {
    ...
    if (
      ...
      // This transaction should only be invoked by
      throw new Error(
        `${userId} does not have access to receive shipment for order ${orderId}`
      );
  ...
  async queryOrder(ctx, orderId) {
    ...
    await ctx.stub.setEvent(
      EVENT_TYPE,
      Buffer.from(JSON.stringify(queryEvent))
    );
    ...
    if (
      ...
      throw new Error(
        `${userId} does not have access to the details of order ${orderId}`
      );
  ...
  async getOrderHistory(ctx, orderId) {
    ...
    if (
      userType == "customer" &&
      order.currentOrderState != OrderStates.SHIPMENT_RECEIVED
    )
      throw new Error(
        `Information about order ${orderId} is not available to ${userId} yet. Order status needs to be SHIPMENT_RECEIVED.`
      );
  ...
  async deleteOrder(ctx, orderId) {
    ...
    if (
      ...
      throw new Error(
        `${userId} does not have access to delete order ${orderId}`
      );
